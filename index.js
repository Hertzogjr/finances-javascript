...
function addTransaction(){
  if(descriptionInput.value.trim() === "" || valueInput.value === "" || typeInput.value === "")
    return alert("Fill all inputs")

    const descriptionOutSpaces = descriptionInput.value.trim()
    const descriptionFormatted = descriptionOutSpaces
    .split('')[0]
    .toUpperCase() + descriptionOutSpaces
    .substring(1) 

  const pureValue = valueInput.value.replace('R$', "").replace('.', '').replace(',', '.')
...
const getTotals = () => {
  ...
  const totalIncomes = () => amountIncomes().reduce((acc, { value }) => acc + Number(value) ,0)
  
  const amountExpenses = () =>
    getItemsDb().filter(({ type }) => type === "expense");
  const totalExpenses = () => amountExpenses().reduce((acc, {value}) => acc + Number(value), 0)
...
const formatBrazilianCurrency = (string) => { return string.toLocaleString('pt-br', {
  style: "currency",
  currency: "BRL"
})
};
